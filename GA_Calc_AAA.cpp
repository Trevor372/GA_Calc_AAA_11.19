...
GA_Calc_AAA::GA_Calc_AAA(void) : DefaultGUIModel("GA_Calc_AAA", ::vars, ::num_vars)
{
...
GA_Calc_AAA::~GA_Calc_AAA(void){}

void GA_Calc_AAA::execute(void)
{
...
void GA_Calc_AAA::update(DefaultGUIModel::update_flags_t flag){
	switch(flag){
		case INIT:
...
void GA_Calc_AAA::initParameters() {
	V0 = 0; // mV
	GA_MAX = .012;
	EA = -100;
	rate = 400;
...
void GA_Calc_AAA::solve(double dt, double *y, double V){
	double dydt[2];
	derivs(y, dydt,V);
...
void GA_Calc_AAA::derivs(double *y, double *dydt, double V){
	GA = GACalc;	
	IA = -GACalc*(V - EA)/cm;
	IACell = IA * 2e-9;
